...
# ── Paths ──────────────────────────────────────────────────────────────────────
DB_PATH = os.environ.get("DATABASE_URL", "sqlite:///./data/financial_agent.db").replace("sqlite:///", "")
# Ensure directory exists
os.makedirs(os.path.dirname(DB_PATH) if os.path.dirname(DB_PATH) else ".", exist_ok=True)

CHROMA_PATH = os.environ.get("CHROMA_PATH", "./data/chroma_store")
...
def add_to_global_watchlist(ticker: str, notes: str = "") -> dict:
    """Global watchlist (not session-scoped) for the portfolio view."""
    now = datetime.utcnow().isoformat()
    with get_connection() as conn:
        # Create global_watchlist table if missing
        conn.execute("""CREATE TABLE IF NOT EXISTS global_watchlist (
            ticker TEXT PRIMARY KEY,
            notes TEXT DEFAULT '',
            added_at TEXT NOT NULL
        )""")
        conn.execute(
            "INSERT OR IGNORE INTO global_watchlist (ticker, notes, added_at) VALUES (?, ?, ?)",
            (ticker, notes, now),
        )
    return {"ticker": ticker, "notes": notes, "added_at": now}


def remove_from_global_watchlist(ticker: str) -> bool:
    with get_connection() as conn:
        cur = conn.execute("DELETE FROM global_watchlist WHERE ticker=?", (ticker,))
        deleted = cur.rowcount
    return deleted > 0


def get_global_watchlist() -> list[dict]:
    with get_connection() as conn:
        conn.execute("""CREATE TABLE IF NOT EXISTS global_watchlist (
            ticker TEXT PRIMARY KEY, notes TEXT DEFAULT '', added_at TEXT NOT NULL
        )""")
        rows = conn.execute(
            "SELECT ticker, notes, added_at FROM global_watchlist ORDER BY added_at DESC"
        ).fetchall()
    return [dict(r) for r in rows]